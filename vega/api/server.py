"""
api/server.py
Vega FastAPI backend — the integration hub for Phase 3.

Implements every endpoint from API.md:
...
async def _run_indexing_job(
    ...
    try:
        ...
        loader = RepoLoader(github_token=github_token)
        chunks, file_tree, import_graph = await asyncio.get_event_loop().run_in_executor(
            None,
            lambda: loader.load_repo(repo_url, branch=branch),
        )

        file_count = len(set(c["file"] for c in chunks))
